...
class search
{
	...
	public function doSearch()
	{
		...
		if($userid == ""){
			$sql23 = "`domain_name` LIKE '%$domain%'";
		}elseif($domain == ""){
			$sql23 = "`user_id`='$userid'";
		}else{
			$sql23 = "`domain_name` LIKE '%$domain%' AND `user_id` = '$userid'";
		}
    ...
    public function showresult()
    {
		...
		print '<td><a href="' . WEB_ROOT . 'admin/dns/&domain=' . $row['domain_id'] . '">' . $obj3->domain_name . '</td>';
				print '<td width="70" align="center"><a href="' . WEB_ROOT . 'admin/domain/modify/&id=' . $row['domain_id'] . '">Modify</a></td>';
