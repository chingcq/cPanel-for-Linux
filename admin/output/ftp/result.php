...
class search
{
	...
	public function doSearch()
	{
		...
		$sql23 = "`User` LIKE '%$ftp_username%' AND `user_id`='$userid'";
		}else{
		$sql23 = "`domain_id`='$domain' AND `User` LIKE '%$ftp_username%' AND `user_id` = '$userid'";
		}
    ...
    public function showresult()
    {
		...
		if ($this->numrows > 0) {
			$i = 0;
			foreach ($this->registry['conn']->query($this->sql) as $row){		$sql2 = "SELECT * FROM buga WHERE user_id =" . $row['user_id'];
