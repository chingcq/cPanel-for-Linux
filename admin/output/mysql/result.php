		...
		switch ($action) { 
			case 'deleteEntry' :
				$array = array('type' 			=> 'delete',
							'delete' 		=> array(	0 => 'tbl_mysql|mysql_id'));
				new process($this->registry, $array); 
				echo "<script>document.location='" . WEB_ROOT . "admin/mysql/result'</script>"; 
			break;
		}
class search
{
	...
	public function doSearch()
	{
		...
		@$mysql_dbname	= trim($_POST['txtmysql']);

		if($userid == ""){
			if($mysql_dbname == ""){
			$sql23 = "`domain_id` = '$domain'";
			}if($domain == ""){
			$sql23 = "`mysql_dbname` LIKE '%$mysql_dbname%'";
			}else{
			$sql23 = "`mysql_dbname` LIKE '%$mysql_dbname%' AND `domain_id`='$domain'";
			}
		}elseif($domain == ""){
		$sql23 = "`mysql_dbname` LIKE '%$mysql_dbname%' AND `user_id`='$userid'";
		}else{
		$sql23 = "`domain_id`='$domain' AND `mysql_dbname` LIKE '%$mysql_dbname%' AND `user_id` = '$userid'";
		}
    ...
    public function showresult()
    {
		...
		if ($this->numrows > 0) {
			...
			foreach ($this->registry['conn']->query($this->sql) as $row){
				...
				print '<td>' . $row['mysql_username'] . '</td>';
				print '<td width="70" align="center"><a href="' . WEB_ROOT . 'admin/mysql/modify/&id=' . $row['mysql_id'] . '">Modify</a></td>';
				print '<td width="70" align="center"><a href="javascript:deleteEntry(' . $row['mysql_id'] . ');">Delete</a></td>';
				print '</tr>';
			}
			print "<tr>\n";
