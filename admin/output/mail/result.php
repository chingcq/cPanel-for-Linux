...
class search
{
	...
	public function doSearch()
	{
		...
		if($userid == ""){
			...
			$sql23 = "`email` LIKE '%$ftp_username%'";
			}else{
			$sql23 = "`email` LIKE '%$ftp_username%' AND `domain_id`='$domain'";
			}
		}elseif($domain == ""){
		$sql23 = "`email` LIKE '%$ftp_username%' AND `user_id`='$userid'";
		}else{
		$sql23 = "`domain_id`='$domain' AND `email` LIKE '%$ftp_username%' AND `user_id` = '$userid'";
		}
