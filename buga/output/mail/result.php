...
class search
{
	...
	public function doSearch()
	{
		...
		if($userid == ""){
			...
			$sql23 = "`email` LIKE '%$ftp_username%' AND `user_id`='" . $_SESSION['number'] . "'";
			}else{
			$sql23 = "`email` LIKE '%$ftp_username%' AND `domain_id`='$domain' AND `user_id`='" . $_SESSION['number'] . "'";
			}
		}elseif($domain == ""){
		$sql23 = "`email` LIKE '%$ftp_username%' AND `user_id`='" . $_SESSION['number'] . "'";
		}else{
		$sql23 = "`domain_id`='$domain' AND `email` LIKE '%$ftp_username%' AND `user_id`='" . $_SESSION['number'] . "'";
		}
