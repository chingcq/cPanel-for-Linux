...
<?
		...
		switch ($action) { 
			case 'deleteEntry' :
				...
				echo "<script>document.location='" . WEB_ROOT . "admin/ftp/result'</script>"; 
			break;
		}
class search
{
	...
	public function doSearch()
	{
		...
		$sql23 = "`ftp_username` LIKE '%$ftp_username%' AND `user_id`='$userid'";
		}else{
		$sql23 = "`domain_id`='$domain' AND `ftp_username` LIKE '%$ftp_username%' AND `user_id` = '$userid'";
		}
