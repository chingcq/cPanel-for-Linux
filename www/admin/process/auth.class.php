...
Class auth
{
	...
	public function checkUser()
	{
		...
		else
		{
			 ...
			 */
			$felder = explode("|" , $_SESSION['fake']);
			$this->sql = "SELECT login FROM `klpt` WHERE `login` = '" . $felder[1] . "' AND `password` = '" . $felder[0] . "'";
	...
	public function doLogin()
	{
	    ...
	    else
	    {
			 ...
			 */
	        $this->sql = "SELECT login FROM `klpt` WHERE `login` = '" . $this->username . "' AND `password` = '" . $this->passwordmd5 . "'";
	        $this->result = $this->registry['conn']->query($this->sql);
            $count = $this->result->rowCount();
			/**
			 * If $count is 1 setting session['fake'] with user data.
	        ...
	        if($count == 1)
	        {
				...
				$_SESSION['fake'] = $this->passwordmd5 . "|" . $this->username;
