			...
			case 'mysql':
				if($this->array['type'] == 'add')
				{
					$this->rowcount = array(
										0 => "SELECT domain_id FROM tbl_domain WHERE domain_id = '" . $this->feld['txtDomain'] . "'|0|Domain does not exists",
										1 => "SELECT user_id FROM buga WHERE user_id = '" . $_SESSION['number'] . "'|0|User does not exist",
										2 => "SELECT mysql_dbname FROM tbl_mysql WHERE user_id = '" . $_SESSION['number'] . "' AND mysql_dbname = '" . $this->feld['txtmysql'] . "'|1|mysql user already exists under given domain");
				}
				if($this->array['type'] == 'modify')
				{
	...
	public function Query()
	{
		switch($this->array['class'])
		{
			case 'ftp':
				if($this->array['type'] == 'add')
				{
					$this->query = "INSERT INTO tbl_ftp (user_id, domain_id, ftp_username, ftp_password) VALUES('" . $_SESSION['number'] . "', '" . $this->feld['txtDomain'] . "', '" . $this->feld['txtFTP'] . "', '" . $this->feld['txtFTPpass'] . "')";
				}
				if($this->array['type'] == 'modify')
				{
					$this->query = "UPDATE tbl_ftp SET domain_id='" . $this->feld['txtDomain'] . "', ftp_username='" . $this->feld['txtFTP'] . "', ftp_password=md5('" . $this->feld['txtFTPpass'] . "') WHERE ftp_id='" . $this->feld['c_ftp_id'] . "' AND user_id = '" . $_SESSION['number'] . "'";
				}
			break;
			...
			case 'domain':
				if($this->array['type'] == 'add')
				{
					...
					$this->query = "INSERT INTO tbl_domain (user_id, domain_name) VALUES('" . $_SESSION['number'] . "', '" . $this->feld['txtDomain'] . "');
									INSERT INTO tbl_records (user_id, domain_name, dns_name, record_type, record_name)
									VALUE('" . $_SESSION['number'] . "', '" . $this->feld['txtDomain'] . "', '" . $dns_name[1] . "', 'A', 'www'), ('" . $_SESSION['number'] . "', '" . $this->feld['txtDomain'] . "', '" . $dns_name[1] . "', 'A', 'mail')";
				}
				if($this->array['type'] == 'modify')
				{
					$dns_name = explode("http://", $this->feld['txtDomain']);
					$dns_name = explode("www.", $dns_name[0]);
					$this->query = "UPDATE tbl_domain SET user_id='" . $_SESSION['number'] . "', domain_name='" . $this->feld['txtDomain'] . "' WHERE domain_id='" . $this->feld['c_id'] . "' AND user_id = '" . $_SESSION['number'] . "';
									UPDATE tbl_records SET domain_name = '" . $this->feld['txtDomain'] . "', dns_name = '" . $dns_name[1] . "' WHERE domain_name = '" . $this->feld['c_domain'] . "' AND user_id = '" . $_SESSION['number'] . "'";
				}
			break;
			...
			case 'email':
				if($this->array['type'] == 'add')
				{
					$this->query = "INSERT INTO tbl_email (user_id, domain_id, email_name, email_password) VALUES('" . $_SESSION['number'] . "', '" . $this->feld['txtDomain'] . "', '" . $this->feld['txtEmail'] . "', '" . $this->feld['txtEmailpass'] . "')";
				}
				if($this->array['type'] == 'modify')
				{
					$this->query = "UPDATE tbl_email SET user_id='" . $_SESSION['number'] . "', domain_id='" . $this->feld['txtDomain'] . "', email_name='" . $this->feld['txtEmail'] . "', email_password=md5('" . $this->feld['txtEmailpass'] . "') WHERE email_id='" . $this->feld['c_email_id'] . "' AND user_id = '" . $_SESSION['number'] . "'";
				}
			break;
