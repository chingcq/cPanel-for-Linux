...
<?
		...
		switch ($action) {
			case 'deleteEntry' :
				...
				echo "<script>document.location='" . WEB_ROOT . "buga/ftp/result'</script>";
			break;
		}
class search
{
	...
	public function doSearch()
	{
		...
		if($userid == ""){
			...
			$sql23 = "`ftp_username` LIKE '%$ftp_username%'";
			}else{
			$sql23 = "`ftp_username` LIKE '%$ftp_username%' AND `domain_id`='$domain'";
			}
		}elseif($domain == ""){
		$sql23 = "`ftp_username` LIKE '%$ftp_username%' AND `user_id`='" . $_SESSION['number'] . "'";
		}else{
		$sql23 = "`domain_id`='$domain' AND `ftp_username` LIKE '%$ftp_username%' AND `user_id` = '" . $_SESSION['number'] . "'";
		}
    ...
    public function showresult()
    {
		...
		if ($this->numrows > 0) {
			...
			foreach ($this->registry['conn']->query($this->sql) as $row){
				$sql2 = "SELECT * FROM buga WHERE user_id =" . $_SESSION['number'];
