...
<?
		...
		switch ($action) {
			case 'deleteEntry' :
				...
				echo "<script>document.location='" . WEB_ROOT . "buga/mail/result'</script>";
			break;
		}
class search
{
	...
	public function doSearch()
	{
		...
		if($userid == ""){
			if($ftp_username == ""){
			$sql23 = "`domain_id` = '$domain' AND `user_id`='" . $_SESSION['number'] . "'";
			}if($domain == ""){
			$sql23 = "`email_name` LIKE '%$ftp_username%' AND `user_id`='" . $_SESSION['number'] . "'";
			}else{
			$sql23 = "`email_name` LIKE '%$ftp_username%' AND `domain_id`='$domain' AND `user_id`='" . $_SESSION['number'] . "'";
			}
		}elseif($domain == ""){
		$sql23 = "`email_name` LIKE '%$ftp_username%' AND `user_id`='" . $_SESSION['number'] . "'";
		}else{
		$sql23 = "`domain_id`='$domain' AND `email_name` LIKE '%$ftp_username%' AND `user_id`='" . $_SESSION['number'] . "'";
		}
