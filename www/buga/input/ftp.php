...
Class Input_FTP Extends Input_Base {
	...
	function modify()
	{
        ...
        $sql = "SELECT * FROM tbl_ftp WHERE ftp_id = '" . $id . "' AND user_id = '" . $_SESSION['number'] . "'";
		$stmt = $this->registry['conn']->query($sql);
		$result = $stmt->fetch(PDO::FETCH_OBJ);
		$domain_id=$result->domain_id;
		$ftp_name = $result->ftp_username;

		$_POST['c_ftp_username']=$ftp_username2=$result->ftp_username;
		$_POST['c_ftp_id']=$result->ftp_id;

        $sql = "SELECT * FROM tbl_domain WHERE domain_id = '" . $domain_id . "' AND user_id = '" . $_SESSION['number'] . "'";
		$stmt = $this->registry['conn']->query($sql);
		$result = $stmt->fetch(PDO::FETCH_OBJ);
